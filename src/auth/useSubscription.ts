...
export const useSubscription = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsage = async () => {
      try {
        ...
        // Calculate usage
        const creditsUsed = data?.reduce((sum, record) => 
          sum + (CREDIT_COSTS[record.action as keyof typeof CREDIT_COSTS] || 0), 0
        ) || 0;
  ...
  const canPerformAction = (action: keyof typeof CREDIT_COSTS): 
    ...
    try {
      ...
      if (usage) {
        ...
        setUsage({
          ...
          magicLinksUsed: action === 'magic_link' 
            ? usage.magicLinksUsed + 1 
            : usage.magicLinksUsed,
          canvasScansThisMonth: action === 'canvas_scan'
            ? usage.canvasScansThisMonth + 1
            : usage.canvasScansThisMonth
        });
      }
    } catch (error) {
